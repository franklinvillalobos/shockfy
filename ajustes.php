...
// Intentar cargar avatar existente
$avatarUrl = null;
$avatarCandidates = [
...
foreach ($avatarCandidates as $path) {
  if (file_exists($path)) { $avatarUrl = $path . '?v=' . filemtime($path); break; }
}
...
<body>
  ...
  <script>
    ...
    inputAvatar?.addEventListener('change', () => {
      ...
      if (!okType){ alert('Formato inválido. Usa JPG, PNG o WEBP.'); inputAvatar.value=''; return; }
      if (f.size > 2 * 1024 * 1024){ alert('La imagen supera 2MB.'); inputAvatar.value=''; return; }
