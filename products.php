...
body{
  ...
  background:#f4f6f9;color:#333;padding:40px;
}
body.dark{background:#181818;color:#f1f1f1}
.container{
  max-width:1400px;margin:60px auto;padding:30px;
  background:#fff;border-radius:16px;
  box-shadow:0 8px 20px rgba(0,0,0,.05);
}
body.dark .container{background:#242424}
h1{text-align:center;margin-bottom:25px;font-weight:600}
.search-box,.order-box,.category-box{
  display:flex;justify-content:center;gap:10px;flex-wrap:wrap;margin:10px 0;
}
.search-box input,.order-box select,.category-box select{
  padding:7px 12px;border-radius:8px;border:1px solid #ccc;
}
body.dark input,body.dark select{background:#333;border:1px solid #666;color:#fff}
.products-grid{
  display:grid;
...
body.dark .product-card{background:#303030}
.product-card:hover{
...
.badge.cat{background:#3498db20;color:#3498db}
.stock{
  font-weight:600;margin-top:6px;
}
.stock.ok{color:#2e7d32}
.stock.mid{color:#f9a825}
.stock.low{color:#c62828}
.product-actions{margin-top:12px}
.product-actions a{
  font-size:13px;padding:6px 12px;margin:2px;
...
function render(){
  let t=search.value.toLowerCase(),c=cat.value;
  ...
  const body=document.getElementById('productsBody');body.innerHTML='';f.forEach(e=>body.appendChild(e));
}
...
document.querySelectorAll('.product-img').forEach(i=>i.addEventListener('click',()=>{modal.style.display='flex';img.src=i.src;}));
